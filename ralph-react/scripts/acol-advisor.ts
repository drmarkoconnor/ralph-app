...
export function buildAdvice(board: ParsedBoard): Advice {
	...
	const final_contract =
		fcParsed ||
		({ by: opener, level: 1, strain: 'NT' } as Advice['final_contract'])
...
function buildMainline(
	...
	const hasFit =
		/1[♠♥]/.test(opening) &&
		partnerLengths[opening.endsWith('♠') ? 'S' : 'H'] >= 3
	...
	if (/1[♠♥]/.test(opening)) {
		...
		if (partnerHcp <= 5) {
			seq.push('P')
		} else if (partnerHcp <= 9 && !hasFit) {
			seq.push('1NT')
		} else if (partnerHcp <= 8 && hasFit) {
			seq.push('2' + opening[1])
		} else if (partnerHcp <= 11 && hasFit) {
			seq.push('3' + opening[1])
		} else if (partnerHcp >= 12 && hasFit) {
		...
		// Balanced 12-14
		if (partnerHcp < 8) seq.push('P')
		else if (partnerHcp >= 8 && partnerHcp <= 9) seq.push('2NT')
		else if (partnerHcp >= 10) seq.push('3NT')
	} else {
		// Minor opening simplified: partner responds 1NT with 6-9, raises minor rarely else pass
		if (partnerHcp < 6) seq.push('P')
		else seq.push('1NT')
	}
...
function buildAlternatives(
	...
	if (/1[♠♥]/.test(opening)) {
		...
		alts.push({
			label: 'Off-book 2♦',
			seq: [opening, 'P', '2♦', 'P', '2' + opening[1], 'P', 'P', 'P'],
			prob: 0,
			bullets: [
				'Some players bid 2♦ lightly; ACOL expects 9+ HCP at the 2-level.',
				`Opener returns to ${opening[1]} partscore.`,
				'Risks missing game when opener is very strong.',
			],
		})
		alts.push({
			label: 'Optimistic raise',
			seq: [
				opening,
				'P',
				'2' + opening[1],
			...
			prob: 0,
			bullets: [
				'Thin raise with inadequate support — undisciplined in ACOL.',
				'Game succeeds here only because opener is huge.',
				'Better evaluation by counting points and fit first.',
			],
		})
	} else {
		// Alternative after 1NT opening: invitational raise vs game direct
		alts.push({
			label: 'Conservative sign-off',
			seq: [opening, 'P', 'P', 'P', 'P', 'P', 'P', 'P'],
			prob: 0,
			bullets: [
				'Responder passes with marginal values.',
				'May miss thin game opposite max 1NT.',
				'Balance by counting combined HCP.',
			],
		})
		alts.push({
			label: 'Over-ambitious 3NT',
			seq: [opening, 'P', '3NT', 'P', 'P', 'P', 'P', 'P'],
			prob: 0,
			bullets: [
				'Responder pushes straight to game.',
				'Can fail if opener is minimum or suit stoppers lacking.',
				'Discipline: evaluate intermediates and shape.',
			],
		})
	}
...
function assignProbabilities(lines: AuctionLine[]): void {
	...
	const main = lines[0]
	main.prob = 0.88
	const remaining = 1 - main.prob
	const altShare = remaining / (lines.length - 1)
	for (let i = 1; i < lines.length; i++)
		lines[i].prob = parseFloat(altShare.toFixed(2))
	...
	if (drift !== 0 && lines.length > 1)
		lines[lines.length - 1].prob = parseFloat(
			(lines[lines.length - 1].prob + drift).toFixed(2),
		)
}

function parseContract(call: string): Advice['final_contract'] | null {
	const m = call && call.match(/^(\d)([♠♥♦♣]|NT)$/)
	if (!m) return null
	const level = parseInt(m[1], 10)
	const strainMap: any = { '♠': '♠', '♥': '♥', '♦': '♦', '♣': '♣', NT: 'NT' }
	return { by: 'N', level, strain: strainMap[m[2]] }
}
...
function buildBullets(
	...
	bullets.push(
		`Responder ${HCP[partner]} HCP${major ? ` with ${major === '♠' ? '♠' : '♥'} ${lengths[partner][major === '♠' ? 'S' : 'H']}` : ''} chooses ${seq[2] || 'P'}.`,
	)
	bullets.push(
		`Opener ${HCP[opener]} HCP ${major ? 'supports major and sets game' : 'evaluates distribution'} -> ${finalAction(seq)}.`,
	)
	const losersEst = estimateLosers(HCP[opener], lengths[opener])
	bullets.push(
		`Plan: losers ≈${losersEst}; manage entries & potential danger hand.`,
	)
	return bullets.slice(0, 3)
}

function finalAction(seq: string[]): string {
	return [...seq].reverse().find((c) => c !== 'P') || 'P'
}

function estimateLosers(h: number, lens: any): number {
	// crude: 24-HCP/3 approx, limited to 3-5 range
	const approx = Math.round((24 - h) / 3)
	return Math.min(6, Math.max(2, approx))
}

function partnerOf(seat: string) {
	return seat === 'N' ? 'S' : seat === 'S' ? 'N' : seat === 'E' ? 'W' : 'E'
}

function buildTeacherFocus(
	mainline: AuctionLine,
	HCP: Record<string, number>,
	lengths: any,
	board: ParsedBoard,
): string[] {
	...
	return [
		`Count losers first`,
		`Identify danger hand`,
		`Entry management`,
		`Fit & HCP: ${HCP[opener]} + ${HCP[partner]}`,
	]
}
