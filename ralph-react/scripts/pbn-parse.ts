...
export function parsePbn(text: string): ParsedBoard[] {
	...
	for (const raw of lines) {
		...
		if (m) {
			...
			if (tag === 'Board') {
				if (cur) push()
				cur = { board: val, auction: [] }
			} else if (!cur) cur = { board: val, auction: [] }
			if (!cur) continue
			switch (tag) {
				case 'Dealer':
					cur.dealer = val as any
					break
				case 'Vulnerable':
					cur.vul = val
					break
...
function parseDeal(deal: string): Record<'N' | 'E' | 'S' | 'W', string[]> {
	...
	const hands: Record<'N' | 'E' | 'S' | 'W', string[]> = {
		N: [],
		E: [],
		S: [],
		W: [],
	}
	handTokens.forEach((tok, i) => {
		hands[order[i]] = expandHand(tok)
	})
	return hands
}

function dealerOrderFrom(
	start: 'N' | 'E' | 'S' | 'W',
): ('N' | 'E' | 'S' | 'W')[] {
	const seats: ('N' | 'E' | 'S' | 'W')[] = ['N', 'E', 'S', 'W']
	const idx = seats.indexOf(start)
	return [0, 1, 2, 3].map((k) => seats[(idx + k) % 4])
}
...
function expandHand(pattern: string): string[] {
	...
	const make = (suitChar: string, cards: string) =>
		cards.split('').map((r) => suitChar + r)
	return [
		...make('S', sp || ''),
		...make('H', he || ''),
		...make('D', di || ''),
...
export function suitLengths(
	cards: string[],
): Record<'S' | 'H' | 'D' | 'C', number> {
	return {
		S: countSuit(cards, 'S'),
		H: countSuit(cards, 'H'),
		D: countSuit(cards, 'D'),
...
function countSuit(cards: string[], s: string) {
	return cards.filter((c) => c[0] === s).length
}

export function isBalanced(
	lengths: Record<'S' | 'H' | 'D' | 'C', number>,
): boolean {
	const arr = Object.values(lengths).sort((a, b) => a - b) // ascending
	const shape = arr.join('-')
	return (
		['3-3-3-4', '2-3-4-4', '2-3-3-5'].includes(shape) ||
...
export function hasFiveCardMajor(
	lengths: Record<'S' | 'H' | 'D' | 'C', number>,
): 'S' | 'H' | null {
