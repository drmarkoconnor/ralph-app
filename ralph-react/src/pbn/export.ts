...
function suitBlock(h: {S:string[];H:string[];D:string[];C:string[]}): string {
...
export async function exportBoardPBN(board: Board): Promise<string> {
  ...
  if (board.contract) {
    core.push(
      `[Contract "${board.contract.level}${board.contract.strain}${board.contract.dbl||''}"]`
    )
  }
  if (board.declarer) core.push(`[Declarer "${board.declarer}"]`)

  const ext: string[] = []
  ext.push('[TagSpec "System,Theme,Interf,Lead,DDPar,Diagram,PlayScript,Scoring,DealHash"]')
