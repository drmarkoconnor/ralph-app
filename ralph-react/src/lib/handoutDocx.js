...
const monoRun = (text, size = 16) => new TextRun({ text, font: 'Courier New', size, color: '111827' })

const labelRun = (text, opts = {}) => new TextRun({ text, bold: true, color: '374151', size: 17, ...opts })
...
const seatTitle = (title, isDealer) =>
  new Paragraph({ alignment: AlignmentType.LEFT, spacing: { after: 80 }, children: [
    ...
    ...(isDealer ? [new TextRun({ text: '  (dealer)', size: 18, color: '6b7280' })] : []),
  ]})
...
const seatLine = (suitChar, red, ranks) =>
  new Paragraph({
    ...
    children: [
      new TextRun({ text: suitChar, bold: true, size: 32, color: red ? 'c81e1e' : '111827' }),
      new Tab(),
      monoRun(ranks, 32),
    ],
  })
...
function seatBlock(id, d) {
  ...
  const lines = [
    seatTitle({ N: 'North', E: 'East', S: 'South', W: 'West' }[id] || id, dealerId === id),
...
function miniMakeables(snap) {
  ...
  rows.push(new TableRow({ children: [
    new TableCell({ children: [new Paragraph({ children: [labelRun('Suit', { size: 18 })] })] }),
    ...seats.map((s) => new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [labelRun(s, { size: 18 })] })] })),
  ]}))
  for (const st of strains) {
    const icon = st === 'NT' ? 'NT' : suitIcon(st)
    const iconRun = new TextRun({ text: icon, bold: true, size: 18, color: isRed(st) ? 'c81e1e' : '111827' })
    const cells = [new TableCell({ children: [new Paragraph({ children: [iconRun] })] })]
    for (const seat of seats) {
      const raw = tableData?.[st]?.[seat]
      const v = Number.isFinite(raw) ? Math.max(0, raw - 6) : ''
      cells.push(new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [monoRun(String(v), 18)] })] }))
    }
  ...
  return new Table({
    ...
    rows: [
      new TableRow({ children: [
        new TableCell({ verticalAlign: 'center', children: [new Paragraph('')], borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } } }),
        seatBlock('N', d),
        new TableCell({ verticalAlign: 'center', children: [new Paragraph('')], borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } } }),
      ]}),
      new TableRow({ children: [
    ...
    rows: [
      new TableRow({ children: [
        new TableCell({
          ...
          borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } },
          children: [new Paragraph({ children: [new TextRun({ text: `Board ${String(d.number)}`, bold: true, size: 28 })] })],
        }),
        new TableCell({
          width: { size: 50, type: WidthType.PERCENTAGE },
          borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } },
          children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [smallRun(`Dealer: ${d.dealer || ''}   Vul: ${d.vul || 'None'}`)] })],
        }),
      ]}),
    ],
  })
...
function topMeta(d) {
  ...
  const notesCell = new TableCell({
    ...
    children: [
      ...
      ...notes.map((line) => new Paragraph({ spacing: { after: 60 }, children: [smallRun(String(line))] })),
    ],
  })
  const detailsRows = [
    new TableRow({ children: [
      new TableCell({ borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } }, children: [new Paragraph({ children: [labelRun('Details')] })] }),
    ]}),
    ...kv.map(([k, v]) => new TableRow({ children: [
      new TableCell({ borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } }, children: [new Paragraph({ children: [smallRun(k)] })] }),
      new TableCell({ borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE }, left: { style: BorderStyle.NONE }, right: { style: BorderStyle.NONE } }, children: [new Paragraph({ children: [smallRun(v ? String(v) : '—')] })] }),
    ]})),
  ]
