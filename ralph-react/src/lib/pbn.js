...
export function parsePBN(text) {
  ...
  for (const raw of lines) {
    ...
    if (!line) {
      if (current) { deals.push(current); current = null }
      continue
    }
    ...
    if (m) {
      ...
      current = current || { ext: {} }
      switch (key) {
        case 'Board': current.board = val; break
        case 'Dealer': current.dealer = val.toUpperCase(); break
        case 'Vulnerable': current.vul = val; break
        case 'Deal': current.deal = val; break
        case 'Contract': current.contract = val; break
        case 'Declarer': current.declarer = val.toUpperCase(); break
        case 'Auction': current.auction = []; current.auctionDealer = val.toUpperCase(); inAuction = true; break
        case 'Play': current.play = []; current.playLeader = val.toUpperCase(); inPlay = true; break
        case 'Event':
        case 'Site':
        case 'Date':
        case 'Room':
