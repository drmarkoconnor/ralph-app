...
function buildMainlineAuction(deal, HCP, lengths){
  ...
  const calls = [];
  let openerSeat = null; let openingCall = null;
  // 1. Traverse seats until someone opens (>=12 HCP) else all pass out
  for(let i=0;i<4;i++){
    const seat = order[i];
    const ocall = chooseOpening(HCP[seat], lengths[seat]);
    if(ocall !== 'PASS') { openerSeat = seat; openingCall = ocall; calls.push(unicodeBid(ocall)); break; }
    else calls.push('Pass');
  }
  if(!openingCall){
    // Pass out: need remaining passes to make four
    while(calls.length<4) calls.push('Pass');
    return { label:'Mainline ACOL', seq:calls, opener:null, opening:'PASS', bullets: passOutBullets(HCP) };
  }
  ...
  while(nextIndex < responderIndex){ // fill intermediate seats with Pass (should not normally happen if partner directly after LHO)
    calls.push('Pass');
    nextIndex++;
  }
  const responderCallBase = responderCall(openingCall, HCP[partner], lengths[partner]);
  const responderCallUnicode = responderCallBase==='PASS' ? 'Pass' : unicodeBid(responderCallBase);
  calls.push(responderCallUnicode); nextIndex++;
  // 4. RHO pass
  calls.push('Pass'); nextIndex++;
  // 5. Opener rebid (only if responder did something other than Pass or 1NT into game decision context)
  const rebidBase = openerRebid(openingCall, responderCallBase, HCP[openerSeat], lengths[openerSeat]);
  if(rebidBase){ calls.push(unicodeBid(rebidBase)); }
  // 6. Add passes until three passes after last bid.
  ensureThreePassesAfterLastBid(calls);
  return { label:'Mainline ACOL', seq:calls, opener: openerSeat, opening: unicodeBid(openingCall) };
}
...
function ensureThreePassesAfterLastBid(seq){
  ...
  function isBid(c){ return !/^Pass$/i.test(c); }
  if(!seq.some(isBid)) return; // already all pass-out
  let trailing=0;
  for(let i=seq.length-1;i>=0;i--){ if(/^Pass$/i.test(seq[i])) trailing++; else break; }
  while(trailing<3){ seq.push('Pass'); trailing++; }
}

function passOutBullets(HCP){
  const entries = Object.entries(HCP).sort((a,b)=> b[1]-a[1]);
  return [ 'Passed out: no seat reaches 12 HCP.', `Highest HCP seat: ${entries[0][0]} (${entries[0][1]} HCP).`, 'Use for lead / counting practice.' ];
}

function buildMainlineBullets(mainline, HCP, lengths){
  if(mainline.opening==='PASS' || !mainline.opener) return mainline.bullets || passOutBullets(HCP);
  const opener = mainline.opener; const partner = partnerOf(opener);
  const openingBase = toBase(mainline.opening);
  const responderIdx = mainline.seq.findIndex(c=> !/^Pass$/i.test(c) && c!==mainline.opening && true);
  // Determine responder call from sequence (first non-pass after opening by partner)
  const responderCall = mainline.seq.filter(c=> !/^Pass$/i.test(c)).filter(c=> c!==mainline.opening)[0];
  const bullets=[];
  bullets.push(`Opener ${opener}: ${HCP[opener]} HCP opens ${mainline.opening} (${openingDescription(mainline.opening)})`);
  if(responderCall){
    ...
    if(openingBase==='1NT'){
      ...
      if(/^2[SH]$/.test(base)) expl = `Simple raise showing 6-9 HCP & 3+ ${openingSuit}`;
      else if(/^3[SH]$/.test(base)) expl = `Limit raise ~10-12 HCP & 3+ ${openingSuit}`;
    ...
    bullets.push(`Responder ${partner}: ${HCP[partner]} HCP -> ${responderCall}${expl? ' ('+expl+')':''}`);
  }
  // Plan bullet
...
function buildTeacherFocus(mainline, HCP, lengths, deal){
  ...
  const fitFocus = /1[SH]/.test(openingBase) ? 'Assess major fit early' : (openingBase==='1NT' ? 'Count sure winners early' : 'Search for major fit');
  return [ fitFocus, `Combined HCP: ${combined}`, 'Outline entries & danger hand' ];
}
...
function buildAlternatives(mainline, deal, HCP, lengths, rng){
  ...
  if(/1[SH]/.test(openingBase)){
    ...
    if(mainSeqStr.includes(`4${unicodeBid(openingBase).slice(-1)}`)){
      alts.push(makeAltSequence(mainline, substituteResponder(mainline, responderH<=12? '2'+suit : '3'+suit)));
    } else if(support>=3 && responderH>=13){
      alts.push(makeAltSequence(mainline, substituteResponder(mainline, '4'+suit)));
    }
  } else if(openingBase==='1NT'){
    if(responderH>=10){
    ...
    if(responderH>=10){
      alts.push(makeAltSequence(mainline, substituteResponder(mainline, 'PASS')));
    }
  }
...
function substituteResponder(mainline, newResponderBase){
  ...
  if(baseSeq[responderIdx]) baseSeq[responderIdx] = unicodeBid(newResponderBase==='PASS'? 'PASS': newResponderBase);
  ...
  if(/^4[SH]$/.test(newResponderBaseNorm) || newResponderBaseNorm==='3NT'){
    ...
    for(let i=rebidIdx;i<baseSeq.length;i++){
      if(!/^Pass$/i.test(baseSeq[i])){ baseSeq[i]='Pass'; }
    }
  }
...
function assignProb(lines){
  ...
  const main = lines[0];
  if(lines.length===1){ main.prob=1; return; }
  main.prob = 0.88;
  const remain = 1 - main.prob;
  const share = remain / (lines.length-1);
  for(let i=1;i<lines.length;i++) lines[i].prob = parseFloat(share.toFixed(2));
  // Adjust rounding drift
  const sum = lines.reduce((s,l)=> s+l.prob,0);
  const drift = +(1-sum).toFixed(2);
  if(drift!==0) lines[lines.length-1].prob = +(lines[lines.length-1].prob + drift).toFixed(2);
}
...
export function buildAcolAdvice(deal){
  ...
  const alts = buildAlternatives(mainline, deal, HCP, lengths, rng).map(a=> ({ ...a, bullets: buildAltBullets(a, mainline, HCP, lengths) }));
  ...
  const final_contract = parsed ? { ...parsed, by: declarerSeat } : { by: dealerSeat, level:1, strain:'NT' };
  return { dealHash: hash, board: deal.number||0, meta: { dealer: dealerSeat, vul: deal.vul||'None', system: deal.meta?.system || 'ACOL' }, auctions, recommendation_index: 0, final_contract, teacher_focus: buildTeacherFocus(mainline, HCP, lengths, deal) };
}
...
function buildAltBullets(alt, mainline, HCP, lengths){
  ...
  const bullets=[];
  bullets.push(`Alternative responder action: ${altResponder} vs ${mainResponder}.`);
...
function extractResponderCall(seq, opener){
  ...
  for(let i=0;i<seq.length;i++){
    ...
    if(foundOpening && seat===partnerOf(opener) && !/^Pass$/i.test(call)) return call;
  }
  return null;
}
