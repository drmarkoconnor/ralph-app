...
export async function computeDealKeyFromEditor(current, dealer, vul) {
  ...
  const hands = {
    N: { S: toRanks(current.buckets.N, 'Spades'), H: toRanks(current.buckets.N, 'Hearts'), D: toRanks(current.buckets.N, 'Diamonds'), C: toRanks(current.buckets.N, 'Clubs') },
    E: { S: toRanks(current.buckets.E, 'Spades'), H: toRanks(current.buckets.E, 'Hearts'), D: toRanks(current.buckets.E, 'Diamonds'), C: toRanks(current.buckets.E, 'Clubs') },
    S: { S: toRanks(current.buckets.S, 'Spades'), H: toRanks(current.buckets.S, 'Hearts'), D: toRanks(current.buckets.S, 'Diamonds'), C: toRanks(current.buckets.S, 'Clubs') },
    W: { S: toRanks(current.buckets.W, 'Spades'), H: toRanks(current.buckets.W, 'Hearts'), D: toRanks(current.buckets.W, 'Diamonds'), C: toRanks(current.buckets.W, 'Clubs') },
  }
...
export async function computeMakeableGrid(deal, dealKey) {
  ...
  let result = await Promise.race([
    p,
    new Promise((resolve) => setTimeout(() => resolve({ deal_key: key, error: 'timeout' }), timeoutMs)),
  ])
