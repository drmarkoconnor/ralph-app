...
function toDdTableDealPbn(deal) {
  ...
  const segFor = (seat) => {
    ...
    const segs = suitOrder.map((s) => (bySuit[s].length ? bySuit[s].join('') : '-'))
    return segs.join('.')
  }
  const start = deal.dealer || 'N'
  const order = ['N', 'E', 'S', 'W']
  const startIdx = order.indexOf(start)
  const rotated = [order[startIdx], order[(startIdx + 1) % 4], order[(startIdx + 2) % 4], order[(startIdx + 3) % 4]]
  // However, for ddTable we only need a cards string like DealPBN.remainCards with start seat and four segments
  const pbn = `${rotated[0]}:${segFor(rotated[0])} ${segFor(rotated[1])} ${segFor(rotated[2])} ${segFor(rotated[3])}`
  return { cards: pbn }
}
