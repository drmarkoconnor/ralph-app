				...
				<div className="rounded-lg border bg-white p-2">
						...
						{meta.themeChoice === 'Custom…' && (
							<label className="flex flex-col gap-0.5">
								...
								<input
									...
									onChange={(e) =>
										setMeta((m) => ({ ...m, themeCustom: e.target.value }))
									}
									className="border rounded px-2 py-1"
					...
					<div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
						<div className="flex flex-col gap-1">
							<span className="text-gray-600">
								Notes (paste long text, then Split)
							</span>
							<textarea
						...
						<div className="flex flex-col gap-1">
							...
							<div className="flex items-center gap-2">
								<label className="flex items-center gap-1">
									...
									<select
										value={meta.auctionStart}
										onChange={(e) =>
											setMeta((m) => ({ ...m, auctionStart: e.target.value }))
										}
										className="border rounded px-2 py-1">
										<option>N</option>
										<option>E</option>
								...
								<input
									className="border rounded px-2 py-1 flex-1"
									placeholder="e.g. 1NT Pass 2C Pass 2D Pass 3NT Pass Pass Pass"
									value={meta.auctionText}
									onChange={(e) =>
										setMeta((m) => ({ ...m, auctionText: e.target.value }))
									}
								/>
							</div>
							<span className="text-[10px] text-gray-500">
								Enter space-separated calls; ends with three Passes.
							</span>
						</div>
						<div className="flex flex-col gap-1">
							<span className="text-gray-600">
								Ideal Card Play (PlayScript)
							</span>
							<textarea
								className="border rounded px-2 py-1 h-20"
					...
					<div className="mt-2 flex items-center justify-between">
						<div className="text-[11px] text-gray-600">
							Use the card picker below to build hands; this metadata will be
					...
					{exportPreview && (
						<pre className="mt-2 whitespace-pre-wrap text-[10px] leading-tight bg-gray-50 border border-gray-200 rounded p-2 w-full max-h-24 overflow-y-auto">
							{exportPreview}
						</pre>
					)}
				</div>
